...
public class MySwitch extends CompoundButton {
	...
	public MySwitch(Context context, AttributeSet attrs, int defStyle) {
		...
		mOrientation = a.getInteger(R.styleable.MySwitch_orientation,
				HORIZONTAL);
		...
		mThumbExtraMovement = a.getDimensionPixelSize(
				R.styleable.MySwitch_thumbExtraMovement, 0);
		mThumbTextPadding = a.getDimensionPixelSize(
				R.styleable.MySwitch_thumbTextPadding, 0);
		mTrackTextPadding = a.getDimensionPixelSize(
				R.styleable.MySwitch_trackTextPadding, 0);
		mSwitchMinWidth = a.getDimensionPixelSize(
				R.styleable.MySwitch_switchMinWidth, 0);
		mSwitchMinHeight = a.getDimensionPixelSize(
				R.styleable.MySwitch_switchMinHeight, 0);
		mSwitchPadding = a.getDimensionPixelSize(
				R.styleable.MySwitch_switchPadding, 0);
		...
		int appearance = a.getResourceId(
				R.styleable.MySwitch_switchTextAppearanceAttrib, 0);
		if (appearance != 0) {
	...
	public void setSwitchTextAppearance(Context context, int resid) {
		TypedArray appearance = context.obtainStyledAttributes(resid,
				R.styleable.mySwitchTextAppearanceAttrib);

		ColorStateList colors;
		int ts;

		colors = appearance
				.getColorStateList(R.styleable.mySwitchTextAppearanceAttrib_textColor);
		if (colors != null) {
			mTextColors = colors;
		} else {
		...
		ts = appearance.getDimensionPixelSize(
				R.styleable.mySwitchTextAppearanceAttrib_textSize, 0);
		if (ts != 0) {
		...
		typefaceIndex = appearance.getInt(
				R.styleable.mySwitchTextAppearanceAttrib_typeface, -1);
		styleIndex = appearance.getInt(
				R.styleable.mySwitchTextAppearanceAttrib_textStyle, -1);
	...
	private Layout makeLayout(CharSequence text) {
		return new StaticLayout(text, mTextPaint,
				(int) Math.ceil(Layout.getDesiredWidth(text,
						mTextPaint)), Layout.Alignment.ALIGN_NORMAL, 1.f, 0,
				true);
	}
	...
	protected void onLayout_orig(boolean changed, int left, int top, int right,
			int bottom) {
		// Log.d(TAG, "left=" + left +
	...
	public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (mOrientation == HORIZONTAL) {
			...
			if ((this.mPushStyle) || (!mTextOnThumb)) {
		...
		Log.d(TAG, "onMeasure():mSwitchWidth=" + mSwitchWidth
				+ " mSwitchHeight=" + mSwitchHeight);
	...
	protected void onLayout(boolean changed, int left, int top, int right,
			int bottom) {
		Log.d(TAG, "onLayout()-left=" + left + ",top=" + top + ",right="
				+ right + ",bottom=" + bottom);
		super.onLayout(changed, left, top, right, bottom);
		...
		Log.d(TAG, "mSwitchWidth=" + mSwitchWidth + " mSwitchHeight="
				+ mSwitchHeight);
		Log.d(TAG, "mSwitchLeft=" + mSwitchLeft + " mSwitchRight="
				+ mSwitchRight);
		Log.d(TAG, "mSwitchTop=" + mSwitchTop + " mSwitchBottom="
				+ mSwitchBottom);
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mOrientation == VERTICAL) {
			...
			thumbBoundB = switchInnerTop + thumbPos - mThumbExtraMovement
					+ mThumbHeight;
			// Draw the Thumb
			Log.d(TAG, "thumbBoundT, thumbBoundB=(" + thumbBoundT + ","
					+ thumbBoundB + ")");
			Log.d(TAG, "mSwitchLeft, mSwitchRight=(" + mSwitchLeft + ","
					+ mSwitchRight + ")");
		...
		if (mOrientation == HORIZONTAL) {
			...
			mThumbDrawable.setBounds(thumbBoundL, mSwitchTop, thumbBoundR,
					mSwitchBottom);
			mThumbDrawable.draw(canvas);
